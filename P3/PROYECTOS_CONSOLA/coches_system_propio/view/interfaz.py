...
class Vista:
    ...
    def menu_principal(self):
        ...
        frame_menu = Frame(self.wind, bg="#f0f4f7")
        frame_menu.pack(expand=True, fill=BOTH)

        Label(frame_menu, text="..:: MENÚ PRINCIPAL ::..", font=("Helvetica", 22, "bold"), bg="#f0f4f7").pack(pady=20)

        # Estado de la BD
        status_text = "BD: Conectada" if getattr(conexionBD, 'connected', False) else "BD: No disponible — usando memoria"
        status_color = "green" if getattr(conexionBD, 'connected', False) else "orange"
        Label(frame_menu, text=status_text, fg=status_color, bg="#f0f4f7", font=("Arial", 10, "italic")).pack(pady=5)

        btn_style = {"width":28, "height":2, "bg":"#2b6ea3", "fg":"white", "font": ("Helvetica", 11, "bold")}
        Button(frame_menu, text="Gestión de Autos", command=self.vista_autos, **btn_style).pack(pady=8)
        Button(frame_menu, text="Gestión de Camionetas", command=self.vista_camionetas, **btn_style).pack(pady=8)
        Button(frame_menu, text="Gestión de Camiones", command=self.vista_camiones, **btn_style).pack(pady=8)

        Button(frame_menu, text="Salir", command=self.wind.quit, font=("Arial", 12), bg="#b33", fg="white").pack(pady=25)
